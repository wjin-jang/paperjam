...
def create_icons():
    """Generates the 4 UI icons: Back, Shuffle, Loop, Heart."""
    w, h = 33, 12
    icons = {}
    
    # 1. BACK Icon (Arrow pointing left and up)
    img = Image.open('./assets/back.png')
    icons['back'] = img
    
    # 2. SHUFFLE Icon (Crossed arrows)
    img = Image.open('./assets/shuffle.png')
    icons['shuffle'] = img

    # 3. LOOP Icon (Box/Cycle)
    img = Image.open('./assets/loop.png')
    icons['loop'] = img

    # 4. FAVOURITE Icon (Heart)
    img = Image.open('./assets/heart.png')
    icons['fav'] = img

    return icons

UI_ICONS = create_icons()
